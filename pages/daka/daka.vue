<template>
	<view class="calendar-box">
		...
		<view class="but">
			...
			<button :disabled='thisMonth == month ? false : true'  :style="'background:'+(thisMonth == month ? bgday : '#666')+';'" @click="daySign(dayArr[aheadDay + day - 1])">打卡</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.index = this.aheadDay + this.day - 1
		},
		onShow() {
			this.searchDate()
		...
		methods: {
			...
			daySign(obj) {
				...
				if (this.dayArr[this.index].flag) {
					uni.showToast({
						title: '今日已打卡',
						icon: 'none'
					});
				...
				this.$set(this.dayArr[this.index], 'flag', true);
				this.updateDate()
			...
			searchDate(){
				...
				cloudApi.call({
					...
					success: (res) => {
						if(this.localDate == res.result[0].localDate){
							// this.dayArr[index].flag=false
								this.$set(this.dayArr[this.index], 'flag', true);
						}else{
							// let index = this.aheadDay + this.day - 1
							this.$set(this.dayArr[this.index], 'flag', false);
							// this.dayArr[index].flag=true
						}
						this.Day=res.result[0].Day
						console.log(res)
					}
